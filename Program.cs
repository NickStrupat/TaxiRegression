    ...
    class Program
    {
        ...
        static async Task Main(string[] args)
        {
            ...
            foreach (var regressorType in regressorTypes)
            {
                ...
                // for (var i = 0; i != taxiTripTests.Count; i++)
                // {
                //     if (i % 50_000 == 0)
                //         Console.Write(".");
                //     var taxiTripTest = taxiTripTests[i];
                //     var pred = model.Predict(taxiTripTest.taxiTrip);
                //     taxiTripTest.taxiTripFarePrediction = pred;
                // }
                // Console.WriteLine("Done.");
                // File.WriteAllLines(@".\Data\ModelErrors.txt", taxiTripTests.Select(x =>
                //     (x.taxiTrip.FareAmount - x.taxiTripFarePrediction.FareAmount).ToString()
                // ));
                // Console.WriteLine(taxiTripTests.Min(x => x.taxiTrip.FareAmount - x.taxiTripFarePrediction.FareAmount));
                // Console.WriteLine(taxiTripTests.Max(x => x.taxiTrip.FareAmount - x.taxiTripFarePrediction.FareAmount));
                // Console.WriteLine(taxiTripTests.Average(x => x.taxiTrip.FareAmount - x.taxiTripFarePrediction.FareAmount));
            }
        }
